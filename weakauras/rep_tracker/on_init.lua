...
-- Renown (Dragonflight)
aura_env.getRenownColouredValue = function(
    currentRenown,
    renownThreshold,
    currentRenownLevel,
    maxRenownLevel,
...
aura_env.getColouredValue = function(barMin, barMax, barValue, standingId)
    ...
    local currentStanding =
        general.showCurrentStandingPreExalted and standingId and
...
aura_env.getParagonColouredValue = function(val, threshold, hasReward, standingId)
    ...
    if not (val > threshold) then
        return "|c" .. paragonColour .. adjustedVal .. "/" .. threshold .. "|r"
    end

    local adjustedVal = getParagonValue(val, threshold, hasReward)

    local currentStanding =
        general.showCurrentStandingExalted and standingId and " (" .. _G["FACTION_STANDING_LABEL" .. standingId] .. ")" or
        ""
